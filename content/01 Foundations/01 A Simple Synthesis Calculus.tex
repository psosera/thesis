...
Assuming that we have no top-level bindings to work with (\ie, we are operating in a closed context), we know from the desired goal type, $\mkwd{Bool} → T → T → T$, that the synthesized program should have the shape:
\[
...
Constants $c_k$ all have type $T$ (\rulename{t-ex-base}), and a partial function $\many{v_i ⇒ χ_i}{i < m}$ has type $τ_1 → τ_2$ if all of $v_i$ have type $τ_1$ and all of the $χ_i$ have type $τ_2$.
...
\end{quote}
The two mutually recursive judgments constitute a non-deterministic synthesis procedure where a complete \rulename{irefine} derivation corresponds to a solution of this synthesis problem.
...
\rulename{irefine-arr} says that we can synthesize a lambda leaving a synthesis sub-problem of synthesizing its body.
...
To see this, consider synthesizing at goal type $T → T$ with the example worlds
\begin{align*}
  σ_1 &↦ c_1 ⇒ c_2 \bnfalt c_3 ⇒ c_4 \\
  σ_2 &↦ c_5 ⇒ c_6 \bnfalt c_7 ⇒ c_8 \bnfalt c_9 ⇒ c_{10}.
\end{align*}
If we apply \rulename{irefine-arr}, the resulting set of example worlds $X'$ that we use to synthesize the body of lambda is:
\begin{gather*}
...
\end{align*}
then applying \rulename{irefine-arr} results in the following set:
\begin{gather*}
...
We can assume axiomatic reasoning principles for particular functions~\todo{cite}, but these are merely admissible heuristics in \lsyn{} so we do not consider them here.
\todo{Add a note about where in the thesis we talk about them in more detail.}
...
Note that when generating the argument to an application $I$ in \rulename{eguess-app}, we pass provide the \emph{empty} set of examples.
When $X$ is empty, you can see that the \rulename{irefine} judgment degenerates to simple term enumeration, as well.
...
At base type, $v$ and $χ$ are constants so it suffices to ensure they are the same constant (\rulename{eq-base}).
...
\begin{example}
  ...
  \rulename{irefine-arr} is valid (as long as the goal type is an arrow) and passes along the empty example set ($X' = ·$) when synthesizing the body of the lambda.
  \rulename{irefine-base} is also valid (as long as the goal type is $T$) and allows you to synthesize any constant $c_k$ since $X$ is empty.
  Finally, \rulename{irefine-guess} allows you to synthesize any $E$ because $E ⊢ ·$ holds vacuously.
\end{example}
...
\begin{example}
  ...
  The following \rulename{eguess} derivation generates the application itself (with $Γ = b{:}\mkwd{Bool}, t{:}T, f{:}T$).
  \[
...
\begin{example}
  ...
  \todo{%
    How do we want to conclude this part?
    I believe that $βη$-equivalence is decidable in \stlc{}, so we could perform the required equivalence checks in this simple setting.
    However, this doesn't scale up appropriately to more complicated type systems.
    Furthermore, our idea of enumerating to a certain size does not work because some types are uninhabited and so we'll never produce a program of that type!
    I guess we need to assume some existential definition of consistent examples here.
  }
\end{example}
