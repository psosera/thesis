...
We present the full details of the proofs here to provide a template for reasoning about the metatheory of  more complicated type-directed program synthesis systems we discuss in later chapters.

\section{Auxiliary Properties}

First, we require some auxiliary results
...
\end{proof}
Canonicity of example values allows us to reason about the shape of $χ$s given the type they type check against.
...
\end{lemma}
\begin{proof}
  The external language fragment of \lsyn{} is merely \stlc{} (extended with a base type) which is known to be strongly normalizing~\todo{cite}.
\end{proof}
The satisfication judgment (\rulename{satisfies}) relies on evaluation to values, and so whenever we need to show that a program satisfies a set of examples, notably in completeness, we implicitly rely on strong normalization to guarantee that evaluation produces a value.
...
\autoref{lem:type-preservation-of-apply} and \autoref{lem:type-preservation-of-example-world-concatenation} allow us to conclude that the \rulename{irefine-arr} rule produces a well-typed example set ($Χ'$) for its synthesis sub-problem.
...
Intutively, type soundness follows from the fact that we derived the synthesis algorithm from the type checking judgment.
...
In general, to prove example soundness, we need to show that each \rulename{irefine} rule produces well-typed example sets to invoke our inductive hypotheses.
\autoref{lem:type-preservation-of-apply} and \autoref{lem:type-preservation-of-example-world-concatenation} allow us to conclude this for arrow types.
When we introduce additional types in later chapters, we will need to show similar lemmas for each additional introduction form and corresponding \rulename{irefine} rule.
    ...
    \item[Case \normalfont{\rulename{t-Ilam}}:]
      ...
      \begin{itemize}
        ...
        \item $I_1 ⊢ Χ'$.
          ...
          Unfolding the definition of \rulename{satisfies} for $I ⊢ Χ$ and that example world:
          \[
            σ(λx{:}τ_1.\,I_1) ⟶ λx{:}τ_1.\,σ(I_1) ∧ λx{:}τ_1.\,σ(I_1) ≃ pf.
          \]
          If $pf = \many{v_i ⇒ χ_i}{i < m}$, then by \rulename{eq-lam-pf}, we know that
          \[
