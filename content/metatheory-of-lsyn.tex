...
In general, to prove example soundness, we need to show that each \rulename{irefine} rule produces well-typed example sets in order to invoke our inductive hypotheses.
...
The first theorem is strong normalization which states that every term evaluates to a value (\ie, terminates).
...
Finally, we need to show the examples that the \rulename{irefine} rules create preserve example satisfaction.
...
\label{lem:satisfaction-preservation-of-apply}
  If $λx{:}τ.\,I ⊨ \many{σ_i ↦ pf_i}{i < n}$ then $I ⊨ X'$ where $X' = \mfun{apply}(x, σ_i ↦ pf_1) \concat … \concat \mfun{apply}(x, σ_n ↦ pf_n)$.
\end{lemma}
\begin{proof}
  Consider a single example world $σ ↦ pf$.
  Unfolding the definition of \rulename{satisfies} for $λx{:}τ.\,I ⊢ Χ$ and that example world:
  \[
    σ(λx{:}τ_1.\,I) ⟶ λx{:}τ_1.\,σ(I) ∧ λx{:}τ_1.\,σ(I) ≃ pf.
  \]
