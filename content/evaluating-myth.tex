...
Furthermore, on top of these scrutinees, as we increase the scrutinee size $s$, we also increase the likelihood of generating scrutinees that are equivalent to previously generated scrutinees.
...
Notice that because the overspecialization contains no $E$-guessed terms, our initial refinement tree would contain this program immediately and we would be done!
...
In fact, by admitting larger $E$-terms, we use more variables and function applications which, by our Minimum Programming Principle (\autoref{subsec:the-minimum-program-principle}) makes it more likely that our synthesized program generalizes to the behavior that the user intends.
Furthermore, the cost of synthesizing $E$-terms of modest size is very small.
It is not until we reach the critical point in the exponential cliff that generation time jumps from milliseconds to minutes.
...
In particular, it only appends a $0$ onto $l_1$ in the $\mCons$ case, completely ignoring $l_2$ in the process.
...
\begin{description}
  ...
  \item[Lists:]
    ...
    \mlsyn{} and \myth{} do not have polymorphism.
...
\autoref{tbl:myth-raw-benchmarks} presents the complete performance data of running \myth{} over our benchmark suite.
...
The triply nested match is relatively quick to synthesize in the minimal context because we only have the $\mkwd{compare}$ function with which to make complex match scrutinees.
...
The other cases where the execution time explodes, \eg, \texttt{tree\_binsert} and \texttt{list\_sorted\_insert}, have similar problem where deep matching is necessary, but the space of possible matches explodes under the extended context.
