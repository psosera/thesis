...
We also define a small-step, call-by-value operational semantics for the language using evaluation contexts $ℰ$~\todo{cite---felleisen}.
...
\todo{Cite---lambda term counting work.}
...
\todo{%
  Not quite true.
  Our formulation gives $β$-equivalent normal forms wrt full reduction semantics, not the call-by-value semantics that we've prescribed.
}
...
Furthermore, this ambiguity can be mitigated by smart interface design decisions, \eg, displaying the top five synthesis results rather than a singular result and allowing the user to choose~\todo{cite}.
...
In particular, $E$ and $I$ terms are checked in a bidirectional type checking style~\todo{cite---benjamin}.
...
\begin{example}
  ...
  \todo{%
    ...
    We should write up the contradictory examples check, …, probably just introduce it earlier and reference it here again.
  }
\end{example}
