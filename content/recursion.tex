...
\begin{description}
  \item[Structural recursion:]
    ...
    This is strictly more constraining than Coq which performs some $β$-reductions to admit more programs~\todo{cite}, but is much easier to formalize and implement in practice.
...
\end{align*}
\todo{%
  Probably should present the examples in this form:
    \begin{align*}
      \mtrue  ⇒ (\mtrue ⇒ \mtrue \bnfalt \mfalse ⇒ \mfalse) \\
      \mfalse ⇒ (\mtrue ⇒ \mfalse \bnfalt \mfalse ⇒ \mfalse)
    \end{align*}
  consistent with how we handle binary function examples in \mlsyn{}.
  Will require a bit of explaination, expanding on the discussion in the previous section, to justify why we do this.
}
...
\todo{Hrm, do we need an explicit rule stating that $\mNoMatch$ is compatible with nothing, not even itself?}
